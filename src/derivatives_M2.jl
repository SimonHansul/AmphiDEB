...
"""
function growth_M2!(du, u, p, t)::Tuple{Real,Real}
    ...
    # in Model M2, the residual assimilation flux also needs to pay maintenance costs

    dS_mt = sig(
        du.A, 
        du.M, 
        -(du.M / p.eta_SA - du.A), 
        u.metamorph * eta_AS * u.y_j[1] * u.y_jP[1] * (du.A - du.M)
    )
...
"""
function maturation_M2!(du, u, p, t, kappa)::Nothing
    ...
    dH_mt = clipneg(((1 - kappa)/kappa) * du.A - du.J) 
...
function metamorphic_reserve_M2!(du, u, p, t, eta_AS, kappa)::Nothing
    ...
    dE_mt_mt = -((1 - kappa)/kappa * du.A) # during metamorphosis, while there is still E_mt left, it will be used to fuel maintenance and maturation
...
function Amphibian_DEB_M2!(du, u, p, t)::Nothing
    ...
    eta_AS, kappa = growth_M2!(du.ind, u.ind, p.ind, t)
    maturation_M2!(du.ind, u.ind, p.ind, t, kappa)
